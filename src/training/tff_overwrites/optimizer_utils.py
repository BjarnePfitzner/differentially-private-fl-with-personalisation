...
class ClientDeltaFn(object, metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def __call__(self, dataset, initial_weights, client_state):
...
def _build_one_round_computation(
    ...
    client_state_type = ClientState(
        class_weights=type_conversions.type_from_tensors([0.5, 0.5]))

    @computations.tf_computation(dataset_type, model_weights_type, client_state_type)
    @tf.function
    def _compute_local_training_and_client_delta(dataset, initial_model_weights, client_state):
        ...
        client_output = client_delta_fn(dataset, initial_model_weights, client_state)
    ...
    @computations.federated_computation(
        ...
        computation_types.FederatedType(client_state_type, placements.CLIENTS),
    )
    def one_round_computation(server_state, federated_dataset, federated_client_states):
        ...
        client_outputs = intrinsics.federated_map(
            ...
            (federated_dataset, broadcast_output.result, federated_client_states))
