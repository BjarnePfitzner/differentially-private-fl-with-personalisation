...
from src.training.tff_overwrites import optimizer_utils
...
class ClientFedAvg(optimizer_utils.ClientDeltaFn):
    ...
    def __call__(self, dataset, initial_weights, client_state):
        model = self._model
        model._model.set_class_weights(client_state.class_weights)
        ...
        def reduce_fn(num_examples_sum, batch):
            ...
            with tf.GradientTape(persistent=True) as tape:
                output = model.forward_pass(batch, training=True)  # todo add sample_weights here
            ...
            grads_and_vars = optimizer._compute_gradients(output.loss, model.weights.trainable, tape=tape)
