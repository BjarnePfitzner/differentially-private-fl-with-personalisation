        ...
        class _WeightedMeanLossMetric(tf.keras.metrics.Mean):
            ...
            def update_state(self, y_true, y_pred, sample_weight=None):
                ...
                if len(self._loss_fns) == 1:
                    batch_loss = self._loss_fns[0](y_true, y_pred, sample_weight=sample_weight)
                else:
                    batch_loss = tf.zeros(())
    ...
    def _forward_pass(self, batch_input, training=True):
        ...
        if y_true is not None:
            if self._class_weights is not None:
                sample_weight = tf.gather(self._class_weights, y_true)
            if len(self._loss_fns) == 1:
                ...
                batch_loss = tf.add_n([loss_fn(y_true=y_true, y_pred=predictions, sample_weight=sample_weight)] +
                ...
                for i in range(len(self._loss_fns)):
                    ...
                    batch_loss += loss_wt * loss_fn(
                        y_true=y_true[i], y_pred=predictions[i], sample_weight=sample_weight)
        else:
            batch_loss = None
        ...
        self._loss_metric.update_state(y_true=y_true, y_pred=predictions, sample_weight=sample_weight)
