...
#from tensorflow.keras.metrics import SensitivitySpecificityBase


class SensitivitySpecificityBase(tf.keras.metrics.Metric, metaclass=abc.ABCMeta):
    ...
    def __init__(
        self, num_thresholds=200, class_id=None, name=None, dtype=None
    ):
...
class FBetaScore(tf.keras.metrics.Metric):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        if len(y_pred.shape) > 1 and y_pred.shape[1] > 1:
            y_pred = tf.cast(tf.expand_dims(tf.argmax(y_pred, axis=1), -1), dtype=tf.float32)
...
class CalibratedFBetaScore(SensitivitySpecificityBase):
    r"""Computes F-1 Score.
    ...
    def result(self):
        ...
        add_values = (tf.math.square(self.beta) * precisions) + recalls
        means = tf.math.divide_no_nan(mul_values, add_values)
        f1_scores = means * (1 + tf.math.square(self.beta))

        best_idx = tf.math.argmax(f1_scores)
        return f1_scores[best_idx], recalls[best_idx], precisions[best_idx], tf.gather(self.thresholds, best_idx)
...
class CalibratedF1Score(CalibratedFBetaScore):
    r"""Computes F-1 Score.
