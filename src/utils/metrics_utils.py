...
def deterministic_unsorted_segment_sum(data, segment_ids, num_segments):
    casted_segment_ids = tf.cast(segment_ids, tf.int64)
    result_tensor = tf.zeros(data.shape[1:] + [num_segments], dtype=tf.float32)
    i = 0
    for single_data in data:
    #for i in tf.range(data.shape[0]):
        delta = tf.SparseTensor([[casted_segment_ids[i]]], [single_data], result_tensor.shape)
        #delta = tf.SparseTensor([[casted_segment_ids[i]]], [data[i]], result_tensor.shape)
        result_tensor = result_tensor + tf.sparse.to_dense(delta)
        i+=1
...
def _update_confusion_matrix_variables_optimized(
    ...
    if multi_label:
        ...
        def gather_bucket(label_and_bucket_index):
            ...
            return deterministic_unsorted_segment_sum(
                data=label,
        ...
        tp_bucket_v = deterministic_unsorted_segment_sum(
            data=true_labels,
            segment_ids=bucket_indices,
            num_segments=num_thresholds,
        )
        fp_bucket_v = deterministic_unsorted_segment_sum(
            data=false_labels,
            segment_ids=bucket_indices,
            num_segments=num_thresholds,
        )
