...
def build_personalization_eval(model_fn,
    ...
    client_input_type = collections.OrderedDict(
        ...
        val_data=computation_types.SequenceType(element_tff_type),
...
def _compute_p13n_metrics(model_fn, initial_model_weights, train_data, val_data,
    ...
    def loop_and_compute():
        ...
        for name, personalize_fn in personalize_fns.items():
            ...
            p13n_metrics[name] = personalize_fn(model, train_data, val_data, test_data, context)
        return p13n_metrics
